...
func TestShouldRetry(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rc := &requestContext{}
			result := rc.shouldRetry(tt.err, tt.statusCode, 100*time.Millisecond, tt.retryConfig, "", nil)
...
func TestShouldRetryErrorMessageMatching(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := rc.shouldRetry(err, 400, 100*time.Millisecond, config, "", nil)
...
func TestShouldRetryWithWrappedErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rc := &requestContext{}
			// Use status code 400 to avoid the "no response" fallback (statusCode == 0)
			result := rc.shouldRetry(tt.err, 400, 100*time.Millisecond, tt.retryConfig, "", nil)
			require.Equal(t, tt.expected, result)
		})
	}
}
...
func TestShouldRetryStatusCodeBoundaries(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rc := &requestContext{}
			result := rc.shouldRetry(nil, tt.statusCode, 100*time.Millisecond, config, "", nil)
			require.Equal(t, tt.expected, result)
		})
	}
}
...
func TestShouldRetryPriorityOrder(t *testing.T) {
	...
	result := rc.shouldRetry(nil, 503, 100*time.Millisecond, config, "", nil)
	...
	result2 := rc.shouldRetry(nil, 200, 100*time.Millisecond, config2, "", nil)
	...
	result3 := rc.shouldRetry(errors.New("timeout"), 503, 100*time.Millisecond, config3, "", nil)
...
func TestErrorStringMatching(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Use status code 400 to avoid the "no response" fallback (statusCode == 0)
			timeoutMatch := rc.shouldRetry(err, 400, 100*time.Millisecond, timeoutConfig, "", nil)
			...
			// Use status code 400 to avoid the "no response" fallback (statusCode == 0)
			connMatch := rc.shouldRetry(err, 400, 100*time.Millisecond, connConfig, "", nil)
...
func TestLowercaseConversion(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		result := rc.shouldRetry(errors[i], 400, 100*time.Millisecond, timeoutConfig, "", nil)
		require.True(t, result, "Error '%s' should match timeout pattern", errors[i].Error())
	}

	// Last 3 errors should match connection
	for i := 4; i < 7; i++ {
		result := rc.shouldRetry(errors[i], 400, 100*time.Millisecond, connConfig, "", nil)
...
func TestMultipleKeywordsInError(t *testing.T) {
	...
	timeoutResult := rc.shouldRetry(mixedErr, 400, 100*time.Millisecond, timeoutConfig, "", nil)
	require.True(t, timeoutResult, "Error with 'timeout' should match timeout condition")

	// Should also match connection condition
	connResult := rc.shouldRetry(mixedErr, 400, 100*time.Millisecond, connConfig, "", nil)
	...
	bothResult := rc.shouldRetry(mixedErr, 400, 100*time.Millisecond, bothConfig, "", nil)
...
func TestActualErrorStringsFromLogs(t *testing.T) {
	...
	for _, tt := range realWorldErrors {
		t.Run(tt.err.Error(), func(t *testing.T) {
			// Use status code 400 to avoid the "no response" fallback (statusCode == 0)
			timeoutMatch := rc.shouldRetry(tt.err, 400, 100*time.Millisecond, timeoutConfig, "", nil)
			connMatch := rc.shouldRetry(tt.err, 400, 100*time.Millisecond, connConfig, "", nil)
...
func TestContextDeadlineExceededSpecifically(t *testing.T) {
	...
	result := rc.shouldRetry(context.DeadlineExceeded, 400, 100*time.Millisecond, config, "", nil)
	...
	timeoutResult := rc.shouldRetry(timeoutErr, 400, 100*time.Millisecond, config, "", nil)
...
func TestMaxRetryLatency(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := rc.shouldRetry(tt.err, tt.statusCode, tt.requestDuration, tt.retryConfig, "", nil)
			require.Equal(t, tt.expected, result, tt.description)
		})
	}
}
...
func TestMaxRetryLatencyRealWorldScenarios(t *testing.T) {
	...
	t.Run("fast_5xx_failure", func(t *testing.T) {
		...
		result := rc.shouldRetry(errors.New("bad gateway"), 502, 50*time.Millisecond, config, "", nil)
	...
	t.Run("slow_timeout", func(t *testing.T) {
		...
		result := rc.shouldRetry(context.DeadlineExceeded, 400, 10*time.Second, config, "", nil)
	...
	t.Run("archival_query_with_higher_budget", func(t *testing.T) {
		...
		result := rc.shouldRetry(errors.New("service unavailable"), 503, 3*time.Second, config, "", nil)
	...
	t.Run("multiple_fast_failures", func(t *testing.T) {
		...
		for i := 0; i < 3; i++ {
			result := rc.shouldRetry(errors.New("internal server error"), 500, 100*time.Millisecond, config, "", nil)
