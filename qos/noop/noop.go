...
func (n *NoOpQoS) UpdateFromExtractedData(endpointAddr protocol.EndpointAddr, data *qostypes.ExtractedData) error {
	...
	if blockHeight > n.perceivedBlockHeight && !qos.IsPlausibleBlockHeight(blockHeight, n.perceivedBlockHeight) {
		n.logger.Warn().
	...
	} else if blockHeight > n.perceivedBlockHeight {
		n.logger.Debug().
			Str("endpoint", string(endpointAddr)).
...
func (n *NoOpQoS) StartBackgroundSync(ctx context.Context, syncInterval time.Duration) {
	...
	syncEndpointBlocksFromRedis := func() {
		...
		for addr, ep := range n.endpointStore.endpoints {
			if ep.blockHeight != 0 {
	...
	sweepStaleEndpoints := func() {
		removed := n.endpointStore.sweepStaleEndpoints(defaultEndpointTTL)
		if len(removed) == 0 {
			return
		}
		...
		defer cancel()
		if err := n.reputationSvc.RemoveEndpointBlockHeights(sweepCtx, serviceID, removed); err != nil {
			n.logger.Warn().
				Err(err).
				Str("service_id", string(serviceID)).
